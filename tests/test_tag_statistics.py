...
class TestGetTagStatistics:
    ...
    def test_entries_without_tags(self, temp_db: Database, sample_template: Template) -> None:
    ...
    def test_single_project_tags(self, temp_db: Database, sample_template: Template) -> None:
        """Tags counted correctly for single project."""
        project = "/test/project"

        # First create tags in project_tags table
        temp_db.create_tag(project, "authentication", "test_agent")
        temp_db.create_tag(project, "security", "test_agent")
        temp_db.create_tag(project, "bug-fix", "test_agent")
        ...
        result = temp_db.get_tag_statistics(project=project)
    ...
    def test_multiple_projects(self, temp_db: Database, sample_template: Template) -> None:
        """Tags aggregated correctly across multiple projects."""
        # First create tags in project_tags table
        temp_db.create_tag("/project/a", "deployment", "test_agent")
        temp_db.create_tag("/project/a", "testing", "test_agent")
        temp_db.create_tag("/project/b", "deployment", "test_agent")
        ...
        # Query all projects - note: now returns per-project tag counts
        result = temp_db.get_tag_statistics(project=None)
        # deployment appears in both projects (1 each), testing in one
    ...
    def test_sorting_by_count_then_alphabetically(
        ...
        # First create tags in project_tags table
        temp_db.create_tag(project, "zebra", "test_agent")
        temp_db.create_tag(project, "alpha", "test_agent")
        temp_db.create_tag(project, "beta", "test_agent")
    ...
    def test_multiple_tags_per_entry(self, temp_db: Database, sample_template: Template) -> None:
        ...
        temp_db.create_tag(project, "security", "test_agent")
        temp_db.create_tag(project, "production", "test_agent")
...
class TestTagListCLI:
    ...
    def test_tag_list_formatting(
        self, temp_db: Database, sample_template: Template, monkeypatch
    ) -> None:
        ...
        # First create tags in project_tags table
        temp_db.create_tag(project, "authentication", "test_agent")
        temp_db.create_tag(project, "bug", "test_agent")
        ...
        runner = CliRunner()
        result = runner.invoke(main, ["tag", "list", "--project", project])
    ...
    def test_tag_list_path_normalization(
        ...
        # First create tag in project_tags table
        ...
        entry = JournalEntry(
            ...
            template_version=sample_template.version,
            agent="test_agent",
            project=project,  # Normalized format
            field_values={"title": "Entry 1"},
            tags=["test-tag"],
        )
        temp_db.create_entry(entry, sample_template)
